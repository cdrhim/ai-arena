-- SparkClaw AI Arena: replace category-based benefit survey with a two-field solution request.
-- Existing response history stays readable; new responses persist a solution name and details.
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'sc_arena_benefit_need_surveys_solution_name_length'
      and conrelid = 'public.sc_arena_benefit_need_surveys'::regclass
  ) then
    alter table public.sc_arena_benefit_need_surveys
      add constraint sc_arena_benefit_need_surveys_solution_name_length
      check (solution_name is null or char_length(solution_name) between 2 and 120);
  end if;
end;
$$;

create or replace function public.sc_arena_submit_solution_need_survey(
...
begin
  ...
  if char_length(trim(coalesce(p_solution_name, ''))) not between 2 and 120 then
    raise exception 'solution name must be between 2 and 120 characters';
  end if;
  if char_length(trim(coalesce(p_solution_details, ''))) not between 10 and 1000 then
    raise exception 'solution details must be between 10 and 1000 characters';
  end if;
...
create or replace function public.sc_arena_latest_solution_need_survey(
  p_workspace_slug text,
  p_user_id uuid
)
returns jsonb
...
revoke all on function public.sc_arena_submit_solution_need_survey(text, uuid, text, text, text, text, text)
from public, anon, authenticated;
revoke all on function public.sc_arena_latest_solution_need_survey(text, uuid)
from public, anon, authenticated;

grant execute on function public.sc_arena_submit_solution_need_survey(text, uuid, text, text, text, text, text)
to service_role;
grant execute on function public.sc_arena_latest_solution_need_survey(text, uuid)
to service_role;

comment on column public.sc_arena_benefit_need_surveys.solution_name is
  'Member-supplied solution name for the simplified private solution request form.';
