...
test("comparison action presents Clawee as an agentic evidence analysis", () => {
  assert.match(html, /id="runCompareButton"[\s\S]*?sparkclaw-logo-blue\.png[\s\S]*?Clawee 클로이 비교 분석[\s\S]*?Agentic comparison engine/);
...
test("comparison states consistently use Clawee agentic language", () => {
  assert.equal((js.match(/AGENTIC COMPARISON/g) || []).length, 3);
  assert.match(js, /에이전트 분석 중/);
  assert.match(js, /클로이 분석 실행 시 생성/);
  assert.match(js, /클로이 분석 완료/);
